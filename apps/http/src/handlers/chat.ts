...
const mapSessionNotActiveError = (
	error: SessionNotActiveError,
): ChatSessionNotFoundRpcError =>
	new ChatSessionNotFoundRpcError({ sessionId: error.sessionId });

const mapNoPendingQuestionError = (
	error: NoPendingQuestionError,
): NoPendingQuestionRpcError =>
				...
				Stream.unwrap(
					Effect.gen(function* () {
						...
						const worktree = yield* storage.worktrees
							.get(params.worktreeId)
							.pipe(
								Effect.mapError((error) => {
									if (error._tag === "WorktreeNotFoundError") {
										return new ChatRpcError({
											message: `Worktree not found: ${params.worktreeId}`,
										});
									}
						...
						yield* storage.sessions
							.update(params.sessionId, { status: "active" })
							.pipe(
								Effect.mapError((error) => {
						...
						return stream.pipe(
							...
							Stream.onDone(() =>
								storage.sessions
									.update(params.sessionId, { status: "paused" })
									.pipe(Effect.ignore),
							),
						);
					}),
			...
			"chat.isActive": (params) =>
				claude
					...
					.pipe(
						Effect.mapError(
							() =>
								new ChatRpcError({ message: "Failed to check session status" }),
						),
					),
		});
	}),
