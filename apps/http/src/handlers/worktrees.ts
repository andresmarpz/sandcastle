...
const mapWorktreeSyncError = (
	error: SyncErrors,
): GitOperationRpcError | DatabaseRpcError => {
	switch (error._tag) {
		case "GitCommandError":
			return mapGitError(error);
		case "DatabaseError":
		...
		return WorktreeRpc.of({
			...
			"worktree.create": (params) =>
				Effect.gen(function* () {
					...
					const repository = yield* storage.repositories.get(
						params.repositoryId,
					);
			...
			"worktree.sync": () =>
				syncWorktrees.pipe(Effect.mapError(mapWorktreeSyncError)),
		});
	}),
);
