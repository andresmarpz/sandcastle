...
const watchedRenderers = new Set();
const warmPool = [];
const POOL_SIZE = 1;
const WARM_COLS = 100;
const WARM_ROWS = 30;
...
const disposeSessionsForRenderer = (webContentsId) => {
...
const watchRenderer = (wc) => {
    if (watchedRenderers.has(wc.id))
        return;
...
const wireSessionEvents = (session) => {
...
const createSession = (sender, opts) => {
    ...
    const session = { id: opts.id, pty, webContentsId: sender.id };
    sessions.set(opts.id, session);
    watchRenderer(sender);
    wireSessionEvents(session);
};
const isPoolable = (opts) => {
...
const scheduleReplenish = () => {
    if (replenishScheduled)
        return;
...
const adoptWarmPty = (warm, sender, opts) => {
...
export const primeWarmPool = () => {
    void userEnvReady().then(initWarmPool);
};
export const registerPtyHandlers = () => {
