...
const runWithService = <A, E>(effect: Effect.Effect<A, E, ConfigService>): Promise<A> =>
...
describe("SandcastleContext.copyFromBase()", () => {
  test("copies files correctly", async () => {
    ...
    const destContent = await fs.readFile(path.join(worktreePath, ".env"), "utf-8");
  ...
  test("copies with different destination name", async () => {
    ...
    const exists = await Bun.file(path.join(worktreePath, ".env.local")).exists();
  ...
  test("creates destination directories", async () => {
    ...
    const exists = await Bun.file(path.join(worktreePath, "nested/dir/config.json")).exists();
    expect(exists).toBe(true);
  });
...
describe("createSandcastleContext()", () => {
  ...
  test("collects logs via onLog callback", () => {
    ...
    const context = createSandcastleContext(getDefaultParams(), logger, entry => {
