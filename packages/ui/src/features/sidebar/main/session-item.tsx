...
export const SessionItem = memo(
	function SessionItem({ session, repositoryId }: SessionItemProps) {
		...
		}, [deleteSession, session.id, session.repositoryId, session.worktreeId]);
	...
	(prev, next) =>
		...
		prev.session.worktreeId === next.session.worktreeId &&
