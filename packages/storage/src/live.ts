			...
			worktrees: {
				...
				create: (input) =>
					Effect.gen(function* () {
						...
						// Check if path already exists
						const existingPath = yield* tryDb(
							"worktrees.create.checkPath",
							() =>
								db
									.query<Record<string, unknown>, [string]>(
						...
						const existingRepo = yield* tryDb(
							"worktrees.create.checkRepo",
							() =>
								db
									.query<Record<string, unknown>, [string]>(
			...
			chatMessages: {
				...
				deleteBySession: (sessionId) =>
					tryDb("chatMessages.deleteBySession", () =>
						db.run("DELETE FROM chat_messages WHERE session_id = ?", [
							sessionId,
						]),
					),
			},
