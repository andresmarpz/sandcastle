...
export const WorktreeRpcHandlers = WorktreeRpc.toLayer(
  Effect.gen(function* () {
    ...
    return WorktreeRpc.of({
      ...
      "worktree.get": (params) =>
        worktree.get(params.repoPath, params.worktreePath).pipe(
          Effect.map(toWorktreeInfo),
          Effect.mapError(mapGetError)
        ),
      ...
      "worktree.prune": (params) =>
        worktree.prune(params.repoPath).pipe(Effect.mapError(mapGitError)),
    })
  })
)
